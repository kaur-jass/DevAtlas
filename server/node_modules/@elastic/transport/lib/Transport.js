...
class Transport {
    ...
    async _request(params, options = {}, otelSpan) {
        ...
        if (headers['content-type'] == null && (connectionParams.body == null || connectionParams.body === '')) {
            headers['content-type'] = this[symbols_1.kJsonContentType];
        }
            ...
            try {
                ...
                switch (error.name) {
                    ...
                    case 'RequestAbortedError': {
                        ...
                        const wrappedError = new errors_1.RequestAbortedError(error.message, result, { ...errorOptions, cause: error });
                    ...
                    case 'TimeoutError':
                        if (!this[symbols_1.kRetryOnTimeout]) {
                            ...
                            this[symbols_1.kConnectionPool].markDead(meta.connection);
                            const wrappedError = new errors_1.TimeoutError(error.message, result, { ...errorOptions, cause: error });
                            this[symbols_1.kDiagnostic].emit('response', wrappedError, result);
                            throw wrappedError;
                        }
                    // should retry
                    ...
                    case 'ConnectionError': {
                        ...
                        // Wrap the error to get a clean stack trace
                        const connectionUrl = (_26 = meta.connection) === null || _26 === void 0 ? void 0 : _26.url.toString();
                        ...
                        const wrappedError = error.name === 'TimeoutError'
                            ? new errors_1.TimeoutError(error.message, result, { ...errorOptions, cause: error })
                            : new errors_1.ConnectionError(connectionErrorMessage, result, { ...errorOptions, cause: error });
                        this[symbols_1.kDiagnostic].emit('response', wrappedError, result);
                        throw wrappedError;
                    }
